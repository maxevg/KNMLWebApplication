...
<body>
    ...
    <form method="post" id="sample-form" action="${pageContext.request.contextPath}/Eigen">
        <div class="container">
            <div>
                 <a>Собственные значения</a>
            </div>
            <div>
                  <a>Введите матрицу</a>
            </div>
...
<script>
    ...
    sampleForm.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            console.log(responseData)
            console.log(responseData.eigenValue)

            removeAllChildren(document.getElementById("answer"))

            var div = document.createElement("div");

            div.innerHTML += "<nobr>Собственные значения: </nobr>"
            document.getElementById("answer").appendChild(div);

            for(let i = 0; i < countOfFields; i++) {
                div.innerHTML += "<nobr>" + responseData.eigenValue[i] + " </nobr>"
