...
<body>
    ...
    <form method="post" id="sample-form" action="${pageContext.request.contextPath}/QR">
        <div class="container">
            <div>
                 <a>QR-разложение</a>
            </div>
...
<script>
    ...
    sampleForm.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            console.log(responseData)
            console.log(responseData.Q.elems)
            console.log(responseData.R.elems)

            removeAllChildren(document.getElementById("answer"))

            var Q = document.createElement("div");

            Q.innerHTML += "<nobr>Matrix Q</nobr>"
            document.getElementById("answer").appendChild(Q);

            for(let i = 0; i < countOfFields; i++) {
                var div = document.createElement("div");
                for (let j = 0; j < countOfFields; j++) {
                     div.innerHTML += "<nobr>" + responseData.Q.elems[i][j] + " </nobr>"
                }
            ...
            R.innerHTML += "<nobr>Matrix R</nobr>"
            document.getElementById("answer").appendChild(R);

            for(let i = 0; i < countOfFields; i++) {
                var div = document.createElement("div");
                for (let j = 0; j < countOfFields; j++) {
                    div.innerHTML += "<nobr>" + responseData.R.elems[i][j] + " </nobr>"
                }
